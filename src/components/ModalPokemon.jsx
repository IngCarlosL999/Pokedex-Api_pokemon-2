...
const ModalPokemon = ({
  ...
  return (
    <section
      className={`fixed top-0 left-0 right-0 h-screen  lg:hidden 
                  transition-opacity duration-500 ${
                    pokemonDetalle?.types && pokemonDetalle?.types.length > 0
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-8 gap-4 text-center overflow-auto hidden-scroll">
          {/* Card 1 */}
          <div
            className={`rounded-lg shadow-lg p-6 flex flex-col border-4 justify-between ${
              pokemonDetalle?.types && pokemonDetalle?.types.length > 0
          ...
          >
            <div className={"flex justify-between items-center mb-4 "}>
              <h2
                className={`font-roboto text-2xl font-bold capitalize ${
                  pokemonDetalle?.types && pokemonDetalle?.types.length > 0
            ...
            <img
              className="pixelated w-32 h-32 object-contain mx-auto scale-[110%] "
              src={pokemonDetalle?.image}
            ...
            <div className="flex space-x-2 mt-2 justify-center capitalize">
              {pokemonDetalle?.types.map((type) => (
                <span
                  key={type}
                  className={`inline-block px-2 py-1 text-white rounded ${colorByTypes[type]}`}
                >
                  {type}
                </span>
              ))}
          ...
          >
            ...
            <section className=" grid grid-cols-2 gap-2  mt-2 text-center">
              <div>
                ...
                <span className="bg-slate-200 block rounded-full mt-2">
                  {pokemonDetalle?.height} cm
                </span>
              </div>
              <div>
                <h4 className="font-bold  capitalize">Weight</h4>
                <span className="  bg-slate-200 block rounded-full mt-2 ">
                  {pokemonDetalle?.weight} g
                </span>
              </div>
            </section>
            ...
            <section className=" text-center mt-4 ">
              ...
              <ul className=" grid grid-cols-2 mt-2 gap-2 capitalize">
                {pokemonDetalle?.abilities.map((ability) => (
                  <li key={ability} className="bg-slate-200 block rounded-full">
                    <span>{ability}</span>
                  </li>
                ))}
              </ul>
          ...
          {/* Card 3 */}
          <div
            className={`rounded-lg shadow-lg p-6 flex flex-col border-4  ${
              pokemonDetalle?.types && pokemonDetalle?.types.length > 0
                ? borderByTypes[pokemonDetalle.types[0]]
                : ""
            }`}
